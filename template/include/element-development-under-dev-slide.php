<div class="swiper-slide with-bg">
    <div class="location a-center animate fadeIn" style="--location-bg: url('<?php echo $location["bg-image"] ?>')">
        <h3 class="font-heading letter-spacing-title-sm size-h3"><?php echo $location["title"] ?></h3>
        <p class="margin-top-24"><?php echo $location["description"] ?></p>
        <p class="margin-top-40"><span class="weight-bold">Bangkok</span>, Thailand</p>
    </div>
</div>